...
def search_text_branch(db: Session, query_terms: list[str], limit: int = 80) -> list[dict[str, object]]:
    ...
    for row in rows:
        ...
        scored.append({**_segment_row_to_dict(row), "text_score": score / len(query_terms), "branch": "text"})
...
def search_object_branch(db: Session, object_filters: list[ObjectFilter], limit: int = 80) -> list[dict[str, object]]:
    if not object_filters:
        return []
    rows = db.execute(select(Segment)).scalars().all()
    ...
    for row in rows:
        ...
        if passed:
            matched.append({**_segment_row_to_dict(row), "object_score": score / len(object_filters), "branch": "object"})
